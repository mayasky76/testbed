<template>
  <div id="app">
    <div v-if="$root.dbloaded">
      <div id="appContent">
        <router-view />
      </div>

      <div id="navigation">
        <svg viewBox="0 0 24 24" height="24" width="24"   @click="showNav=!showNav" class="NavLogoButton">
        ...
        <div id="nav_elements" :class="{'mobileHide': !showNav}">
          <router-link
            v-for="(link, index) in navIcons"
            :key="index"
            :to="link.path"
    ...
    <div v-if="!$root.dbloaded">Loading the database</div>
  </div>
</template>
<script>
export default {
...
right:0px;
bottom:50px;
background-color:var(--side-tool-panel-background);
}
#nav_elements a {
